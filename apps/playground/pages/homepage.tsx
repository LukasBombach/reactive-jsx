...
const Layout: TC = ({ children }) => (
  <div className="grid grid-cols-1 gap-y-10 px-8 py-4 sm:px-16 sm:py-12 sm:gap-y-16 xl:px-24 xl:py-24">{children}</div>
);
...
const ArticleList: TC = ({ children }) => (
  <ul className="grid grid-cols-1 divide-y dark:divide-slate-700 text-sm max-w-screen-sm">{children}</ul>
);
...
const Fin = () => <span className="italic text-slate-300 text-sm">fin</span>;
